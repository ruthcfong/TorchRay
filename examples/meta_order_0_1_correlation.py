...
def meta_correlation(arch="resnet50",
    ...
    for i, (x, y) in enumerate(tqdm(loader)):
        x = x.to(device)
        y = y.to(device)
        saliency_order_0 = saliency_func(model,
                                         x,
                                         y,
                                         **kwargs)
        saliency_order_1 = meta_saliency(saliency_func,
                                          model,
                                          x,
                                          y,
                                          lr=lr,
        ...
        if plot:
            ...
            imarraysc(x)
            plt.axes(ax[1])
            imarraysc(saliency_order_1)
            plt.axes(ax[2])
            imarraysc(saliency_order_0)
            plt.axes(ax[3])
            imarraysc(torch.abs(saliency_order_1 - saliency_order_0))
            plt.pause(1)

        corr, _ = compute_similarity(saliency_order_1, saliency_order_0)
    ...
    np.savetxt(os.path.join(out_dir, f"{prefix}_order_imdb.txt"),
               samples,
               delimiter="\n",
               fmt="%s,%s")
